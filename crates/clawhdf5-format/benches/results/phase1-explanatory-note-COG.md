...
- 30 measured trials per cell (`TRIALS` in the harness). **Caveat**: unlike
  ...
  boxplot figure below. The reported medians are robust to this
...
vs. chunk count (log-log) for the four scenarios whose 30-trial spread is
tight enough to read as a single line: `verify_dataset`, `full_rebuild`,
`flat_verify`, `verify_chunk`. Each point carries a 95% bootstrap CI error
bar (2000 resamples, same protocol as
...
mechanically the same operation. Generated by `phase1-plot-lines-COG.gp`
(`gnuplot phase1-plot-lines-COG.gp`).

[phase1-plot-boxplot-COG.pdf](phase1-plot-boxplot-COG.pdf) —
`extend_merkle`/`update_merkle`, the two scenarios whose raw 30-trial spread
is *not* tight: Tukey box-and-whisker plots (Q1/median/Q3, whiskers capped
at the most extreme point within 1.5×IQR of the box), with every trial
beyond that fence drawn as an individual outlier marker. These two
scenarios get the distributional view instead of a median line because,
unlike the line-chart scenarios above, several of their cells contain
genuine multi-trial cold-start outliers — direct visual evidence for the
no-discarded-warmups caveat below, not just an abstract risk. Generated by
...
The boxplot figure makes the largest-N cell's behavior visible
