...
![Median wall time vs. chunk count, log-log, all 6 chunk-count-dependent scenarios](phase1-plot-COG.png)

Generated by `phase1-plot-COG.gp` (`gnuplot phase1-plot-COG.gp`), which
inlines the same per-cell medians tabulated below — the CSV's 30 raw
trials per cell remain the source of truth; the script only renders the
already-verified summary numbers. `verify_dataset`'s line is plotted but
sits exactly underneath `full_rebuild`'s (both run the identical
