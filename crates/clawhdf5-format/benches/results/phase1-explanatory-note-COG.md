...
- 30 measured trials per cell (`TRIALS` in the harness). **Caveat**: unlike
  ...
  each cell is included in the reported set. This is not just a theoretical
  risk: the `extend_merkle`/`update_merkle` cells show real cold-start
  outliers (e.g. at n=54,608 the slowest `extend_merkle` trial is 0.965 ms
  against a ~0.377 ms median; at n=13,652 five of the 30 trials sit
  3-4x above the median), visible directly as the outlier markers in the
  Figure's bottom panel below. The reported medians are robust to this
  (a median tolerates a handful of high outliers), but it remains a
  deviation from the Statistical Protocol's letter and should be fixed by
  discarding warmups if a future cell's *median itself* — not just its
  outliers — turns out to be warmup-sensitive.
...
[phase1-plot-COG.pdf](phase1-plot-COG.pdf) — two stacked panels, rendered as
a vector PDF for `\includegraphics` in LaTeX (not embedded inline here since
Markdown can't preview a PDF).

**Top panel** — median wall time vs. chunk count (log-log) for the four
scenarios whose 30-trial spread is tight enough to read as a single line:
`verify_dataset`, `full_rebuild`, `flat_verify`, `verify_chunk`. Each point
carries a 95% bootstrap CI error bar (2000 resamples, same protocol as
...
**Bottom panel** — `extend_merkle`/`update_merkle`, the two scenarios whose
raw 30-trial spread is *not* tight: Tukey box-and-whisker plots (Q1/median/
Q3, whiskers capped at the most extreme point within 1.5×IQR of the box),
with every trial beyond that fence drawn as an individual outlier marker.
These two scenarios get the distributional view instead of a median line
because, unlike the top panel's four scenarios, several of their cells
contain genuine multi-trial cold-start outliers — direct visual evidence for
the no-discarded-warmups caveat below, not just an abstract risk.

Generated by `phase1-plot-COG.gp` (`gnuplot phase1-plot-COG.gp`).
...
The Figure's bottom panel makes the largest-N cell's behavior visible
directly rather than just in the median: at n=54,608, `extend_merkle`'s box
(Q1-Q3) spans roughly 0.364-0.486 ms, with three trials landing as outliers
above the box (0.529 ms, 0.535 ms, and a 0.965 ms cold-start trial — about
2.6x the median). That spread is consistent with an L3-resident clone being
more sensitive to cache contention/eviction from other system activity
between trials than the L2-resident clones at smaller N, whose boxes are
comparatively tight. `update_merkle` at the same N shows a narrower spread
(one outlier at 0.383 ms vs. a 0.369 ms median), suggesting the wider
`extend_merkle` spread is at least partly run-to-run noise rather than a
deterministic property of append vs. in-place update — both call the same
`update_leaf` primitive after an identical clone.
