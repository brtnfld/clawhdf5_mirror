...
impl Version {
    ...
    pub fn parse(s: &str) -> Option<Self> {
        // Upstream only lists minors for released 1.x series (0,1,8,10,12,14)
        // plus 2.0/2.1. Add 2.2 so builds against the HDF5 2.x `develop`
        // branch (currently versioned 2.2.0) don't panic here.
        let re =
            Regex::new(r"^(1|2)\.(0|1|2|8|10|12|14)\.(\d\d?)(_|.\d+)?((-|.)(patch)?\d+)?$").ok()?;
